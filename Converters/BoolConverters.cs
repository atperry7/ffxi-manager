...
namespace FFXIManager.Converters
{
    ...
    /// <summary>
    /// Converts null objects to Visibility (null = Hidden, non-null = Visible)
    /// </summary>
    ...
    /// <summary>
    /// Converts zero values to inverted Visibility (zero = Hidden, non-zero = Visible)
    /// </summary>
    public class ZeroToInverseVisibilityConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value is double doubleValue)
                return Math.Abs(doubleValue) < 0.001 ? Visibility.Collapsed : Visibility.Visible;
            if (value is decimal decimalValue)
    ...
    /// <summary>
    /// Converts Action string to Visibility - shows only when Action is "Click"
    /// Used to hide/show coordinate columns in navigation sequence editor
    /// </summary>
    public class IsClickActionToVisibilityConverter : IValueConverter
    {
        public static readonly IsClickActionToVisibilityConverter Instance = new();
